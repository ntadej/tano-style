...
for (( i=0; i<${#apple[@]}; i++ ));
do
    inkscape -o "$dir/favicon/apple_${apple[$i]}.png" -w "${sizes[$i]}" "$dir/src/logo_square.svg"
done
...
for (( i=0; i<${#sizes[@]}; i++ ));
do
    inkscape -o "$dir/favicon/${sizes[$i]}.png" -w "${sizes[$i]}" "$dir/src/logo.svg"
done
...
for (( i=0; i<${#ms[@]}; i++ ));
do
    inkscape -o "$dir/favicon/mstile_${ms[$i]}.png" -w "${sizes[$i]}" "$dir/src/logo_text.svg"
done
